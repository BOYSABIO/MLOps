...
@click.option("--test-images-path", required=True,
              help="Path to test images")
@click.option("--test-labels-path", required=True,
...
def main(model_path, test_images_path, test_labels_path, output_dir):
    ...
    try:
        ...
        x_tensor = torch.tensor(x_test[:1000],
                                dtype=torch.float32).permute(0, 3, 1, 2)
        y_tensor = torch.tensor(np.argmax(y_test[:1000], axis=1),
                                dtype=torch.long)
